...
# Sampling interval
ts = 1/fs
t = np.arange(0,1,ts) # Create the time array from 0 to 1 seconds
...
plt.xlim(0, .05) # Change xlim for display purposes
...
plt.plot(t, x3, 'b')
plt.ylabel('Amplitude')
plt.title('x3 = cos('+str(freq)+'*2$\pi$t)')
plt.xlim(0, .05) # Change xlim for display purposes
...
cutoff=60
order=5
filtered_sine = butter_lowpass_filter(x4,cutoff,fs,order)
